...
import { TelegramUpdateSchema } from '@shared/schemas/external/telegram'
...
export class TelegramAdapter implements MessagingAdapter {
  ...
  constructor(config: TelegramConfig, crypto: CryptoService) {
  ...
  async parseIncomingMessage(request: Request): Promise<IncomingMessage> {
    ...
    const senderIdentifierHash = this.crypto.hmac(senderIdentifier, HMAC_PHONE_PREFIX)
    ...
    if (update.edited_message) metadata.edited = 'true'
