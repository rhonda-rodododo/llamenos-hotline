...
webauthn.post('/login/verify', async (c) => {
  ...
  try {
    ...
    await audit(dos.session, 'webauthnLogin', matched.ownerPubkey, { credId: matched.id }, c.req.raw)
...
webauthn.post('/register/verify', async (c) => {
  ...
  try {
    ...
    await audit(dos.session, 'webauthnRegistered', pubkey, { credId: newCred.id, label: body.label }, c.req.raw)
...
webauthn.delete('/credentials/:credId', async (c) => {
  ...
  if (res.ok) await audit(dos.session, 'webauthnDeleted', pubkey, { credId }, c.req.raw)
  return res
})
