    ...
    async (files: File[]) => {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            prev.map((u, idx) =>
              idx === uploadIndex
                ? {
                    ...
                    status: 'uploading',
                    totalChunks:
                      Math.ceil(encrypted.encryptedContent.length / (5 * 1024 * 1024)) || 1,
                  }
                : u
            )
          )
          ...
          setUploads((prev) =>
            prev.map((u, idx) => (idx === uploadIndex ? { ...u, status: 'failed' } : u))
          )
          toast(
            t('reports.uploadError', {
  ...
  return (
    <div className="space-y-3">
      <button
        ...
        className={`relative flex w-full flex-col items-center justify-center rounded-lg border-2 border-dashed px-4 py-6 transition-colors ${
          isDragOver
        ...
        disabled={disabled}
        aria-label={t('reports.dropzoneLabel', {
