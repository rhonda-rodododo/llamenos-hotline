...
export function StepProviders({ data, onChange, headingRef }: Props) {
  ...
  const hasVoiceOrSms =
    data.selectedChannels.includes('voice') || data.selectedChannels.includes('sms')
  ...
  return (
    <div className="space-y-6">
      ...
      {hasVoiceOrSms && <VoiceSmsProviderForm data={data} onChange={onChange} />}

      {hasWhatsApp && <WhatsAppProviderForm data={data} onChange={onChange} />}

      {hasSignal && <SignalProviderForm data={data} onChange={onChange} />}
    </div>
  )
}
