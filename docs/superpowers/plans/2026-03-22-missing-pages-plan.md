...
- [x] Add `GET /api/calls/:callId` to `src/worker/routes/calls.ts` (new endpoint — does not currently exist):
...
- [x] Create `src/client/routes/calls.$callId.tsx` (TanStack file-based routing):
...
- [x] Update `src/client/routes/calls.tsx` (call history list):
  - Make each row clickable → navigate to `/calls/:callId`
  - Add `<Link>` or `onClick` handler

### 1.4 Note creation from call detail
- [x] Reuse existing `NewNoteForm` component, pre-filled with `callId`
- [x] Notes created from call detail page are associated with this call

### 1.5 i18n keys
- [x] Add to all 13 locale files: `calls.detail.title`, `calls.detail.metadata`, `calls.detail.notes`, `calls.detail.recording`, `calls.detail.audit`

### 1.6 Tests
- [x] Add to `tests/call-flow.spec.ts` or new test:
...
- [x] Check `src/client/routes/settings.tsx` — verify it already handles name, phone, and spoken languages
...
- [x] Ensure `src/client/routes/settings.tsx` includes:
...
- [x] Profile updates use `PATCH /auth/me/profile` in `src/worker/routes/auth.ts` — this endpoint already exists
...
### 2.4 i18n keys
- [x] Add to all 13 locales: `settings.profile.name`, `settings.profile.phone`, `settings.profile.spokenLanguages`, `settings.profile.save`

### 2.5 Tests
- [x] Add to `tests/profile-settings.spec.ts`:
...
- [x] Add `GET /api/notes/:noteId` to `src/worker/routes/notes.ts` (new endpoint — does not currently exist):
...
- [x] Create `src/client/routes/notes.$noteId.tsx`:
...
- [x] Handle unauthorized: volunteer sees 403 if trying to view another volunteer's note

### 3.2 Link from audit log
- [x] Update audit log entries for `noteCreated` events to link to `/notes/:noteId`

### 3.3 Tests
- [x] Add: navigate to `/notes/:noteId` → note renders correctly
- [x] Unauthorized volunteer gets 403 on another volunteer's note
...
- [x] `/calls/:callId` route: renders metadata, notes, recording, audit trail
- [x] Call history list: rows are clickable links
- [x] `/settings`: profile section verified complete with name/phone/language editing
- [x] Profile updates use existing `PATCH /auth/me/profile` endpoint (no duplicate endpoint created)
- [x] Phone edit requires PIN challenge
- [x] `/notes/:noteId` permalink route renders
- [x] Audit log links to note detail where applicable
- [x] All i18n keys added to 13 locales
- [x] `bun run typecheck` passes
- [x] `bun run build` passes
- [x] E2E tests pass for each new page
