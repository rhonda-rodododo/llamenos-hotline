...
- [ ] Add `getRawNote(request, noteId)` — fetches raw note data from API, returns the ciphertext + envelopes without decryption
- [ ] Add `decryptNoteAsVolunteer(rawNote, volunteerNsec)` — imports crypto utilities, decrypts the note envelope using volunteer's secret key, returns plaintext
- [ ] Add `decryptNoteAsAdmin(rawNote, adminNsec)` — decrypts via admin envelope
- [ ] Add `tryDecryptNoteAsOtherVolunteer(rawNote, otherNsec)` — attempts decryption, expects failure
- [ ] Import the actual crypto functions from `src/client/lib/crypto.ts` in test context:
...
- [ ] Add `getRawMessage(request, conversationId, messageId)` — fetches raw message ciphertext + envelopes

### Test-mode crypto exposure
- [ ] In `src/client/lib/auth.tsx` (or root entry point), when `import.meta.env.MODE === 'test'`:
  ```typescript
  import { decryptNoteV2, decryptMessage } from './crypto'
  window.__llamenos_test_crypto = { decryptNoteV2, decryptMessage }
  ```
- [ ] Ensure this is tree-shaken in production builds (guarded by `import.meta.env.MODE === 'test'`)

---

## Phase 1: Note Encryption Tests

- [ ] Create `tests/e2ee-notes.spec.ts`
...
```
- [ ] Implement test using `request.get('/api/notes/:id')` and inspecting JSON directly
...
```
- [ ] Implement via `page.evaluate()` calling `window.__llamenos_test_crypto.decryptNoteV2(rawNote, volunteerNsec)`
...
- [ ] Create `tests/e2ee-messages.spec.ts`
...
```
- [ ] Implement using `request.post('/api/messaging/sms/inbound', ...)` in test
- [ ] May need test env bypass for webhook signature validation
...
- [ ] Create `tests/e2ee-hub-keys.spec.ts`
...
- [ ] Add to `tests/audit-log.spec.ts`:
...
- [ ] `window.__llamenos_test_crypto` exposed in test/dev mode only (not production)
- [ ] `bun run build` still passes (no test code in production bundle)
- [ ] `bun run typecheck` passes
- [ ] Note encryption test: plaintext never appears in raw API response
- [ ] Note decryption test: volunteer and admin can both decrypt correctly
- [ ] Forward secrecy test: two notes have different envelopes
- [ ] Message encryption test: inbound message stored as ciphertext
- [ ] Unauthorized decryption test: fails for non-recipients
- [ ] Hub key rotation test: departed member loses access
- [ ] Audit hash chain test: chain valid after multiple operations
- [ ] `bunx playwright test tests/e2ee-notes.spec.ts` passes
- [ ] `bunx playwright test tests/e2ee-messages.spec.ts` passes
