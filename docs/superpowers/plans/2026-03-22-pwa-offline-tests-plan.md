...
- [x] Create `tests/pwa-offline.spec.ts`
...
```
- [x] Use `page.evaluate(() => navigator.serviceWorker.ready)` with timeout
...
```
- [x] Use `page.evaluate(() => caches.keys())` then inspect cache contents
...
```
- [x] Use `await page.context().setOffline(true)` (Playwright built-in)
- [x] Check `[data-testid="offline-banner"]` visibility
...
```
- [x] This is the "it loads at all" test — critical for volunteers in poor connectivity areas
...
- [x] Extend `tests/notification-pwa.spec.ts` to verify:
...
```
- [x] Use `page.on('console', msg => ...)` to capture console errors
...
- [x] Service worker registers successfully in test environment
- [x] `navigator.serviceWorker.ready` resolves without timeout
- [x] Cache storage contains app shell, excludes /api/*
- [x] Offline banner appears when `page.context().setOffline(true)`
- [x] Offline banner disappears when network restored
- [x] App shell renders from cache when offline
- [x] No console errors from SRI verification
- [x] `bunx playwright test tests/pwa-offline.spec.ts` passes
