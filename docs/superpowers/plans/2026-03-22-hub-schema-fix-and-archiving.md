...
- [ ] Run migration generation:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && DATABASE_URL=postgresql://llamenos:llamenos@localhost:5433/llamenos bunx drizzle-kit generate
  ```
...
- [ ] Apply migration (dev DB must be running — `bun run dev:docker` from worktree):
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && DATABASE_URL=postgresql://llamenos:llamenos@localhost:5433/llamenos bunx drizzle-kit migrate
  ```
...
- [ ] Commit:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && git add src/server/db/schema/settings.ts drizzle/migrations/ && git commit -m "$(cat <<'EOF'
...
- [ ] Run typecheck to catch any type errors introduced by the schema change:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bun run typecheck 2>&1 | head -60
  ```
...
- [ ] Commit:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && git add src/shared/schemas/settings.ts && git commit -m "$(cat <<'EOF'
...
- [ ] Run typecheck to confirm all errors in settings.ts are resolved:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bun run typecheck 2>&1 | grep 'settings'
  ```
...
- [ ] Commit:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && git add src/server/services/settings.ts src/worker/routes/hubs.ts && git commit -m "$(cat <<'EOF'
...
- [ ] Check the `@/lib/api` module to see if `archiveHub` is already exported:
  ...
  ```bash
  grep -n 'archiveHub\|archive' ~/projects/llamenos-hotline/.worktrees/cf-removal/src/client/lib/api.ts
  ```
...
- [ ] Run typecheck:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bun run typecheck 2>&1 | grep 'hubs'
  ```
...
- [ ] Commit:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && git add src/client/routes/admin/hubs.tsx src/client/lib/api.ts && git commit -m "$(cat <<'EOF'
...
- [ ] Commit:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && git add tests/multi-hub.spec.ts && git commit -m "$(cat <<'EOF'
...
- [ ] Run full typecheck:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bun run typecheck
  ```
...
- [ ] Run full build:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bun run build
  ```
...
- [ ] Ensure dev services are running (postgres on port 5433):
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bun run dev:docker
  ```

- [ ] Run the targeted multi-hub test:

  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bunx playwright test tests/multi-hub.spec.ts
  ```
...
- [ ] If any tests fail, check server logs:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bunx playwright test tests/multi-hub.spec.ts --reporter=line 2>&1 | tail -40
  ```
...
- [ ] If all passing, run the full test suite to check for regressions:
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && bunx playwright test
  ```
...
- [ ] Final commit (if any fixup changes were needed):
  ...
  ```bash
  cd ~/projects/llamenos-hotline/.worktrees/cf-removal && git add -p && git commit -m "$(cat <<'EOF'
