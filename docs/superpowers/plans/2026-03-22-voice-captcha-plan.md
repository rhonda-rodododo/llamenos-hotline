...
- [ ] Read `src/worker/routes/telephony.ts` (or `src/server/routes/telephony.ts` post-Drizzle)
  - Confirm `POST /api/telephony/captcha` route exists
  - If missing: add the route (see Phase 2.3)
- [ ] Read `src/worker/durable-objects/settings-do.ts` (or `SettingsService`)
  - Find `/captcha/store` and `/captcha/verify` handlers
  - Document their actual implementation (what they store and return)
- [ ] Read each telephony adapter's `handleCaptchaResponse()` method
...
- [ ] Read `src/worker/telephony/base-adapter.ts` or the `TelephonyAdapter` interface
...
- [ ] Add `generateCaptchaChallenge(callSid: string): Promise<{ digits: string, expiresAt: number }>` to `SettingsService` (or SettingsDO handler):
...
- [ ] Add Zod schema for CAPTCHA storage: `CaptchaChallenge = z.object({ digits: z.string().length(4), expiresAt: z.number(), attempts: z.number() })`

### 2.2 CAPTCHA verification service method
- [ ] Add `verifyCaptchaChallenge(callSid: string, submittedDigits: string): Promise<'pass' | 'fail' | 'expired' | 'retry'>` to `SettingsService`:
...
- [ ] Add `captchaMaxAttempts: number` (default: 2) to `SpamSettings` type in `src/shared/types.ts`
- [ ] Add to Zod schema and DB column / JSON storage
- [ ] Add to settings UI (see Phase 5)
...
- [ ] For **Twilio adapter** (`src/worker/telephony/twilio.ts`):
...
- [ ] Apply same change to **Vonage**, **Plivo**, **Asterisk** adapters (using their respective DSLs: NCCO, PXML, AMI)
- [ ] SignalWire inherits from Twilio — verify it works without changes

### 3.2 TTS challenge script
- [ ] Add CAPTCHA challenge TTS helper `buildCaptchaTwiml(digits: string, language: string): string`:
...
- [ ] Add equivalent for Vonage (NCCO `input` action), Plivo (PXML `GetDigits`), Asterisk (AGI `GET DATA`)
- [ ] Add CAPTCHA TTS strings to IVR language store (all supported languages in `src/shared/languages.ts`)

### 3.3 CAPTCHA webhook route
- [ ] Verify or add `POST /api/telephony/captcha` route in `src/worker/routes/telephony.ts` (or server equivalent):
  ```typescript
  ...
  ```
- [ ] `handleCaptchaResponse(c)` in each adapter:
...
- [ ] Open CAPTCHA / spam settings component in admin settings page
- [ ] Add `captchaMaxAttempts` number input (label: "Max CAPTCHA attempts before rejection", range 1–5, default 2)
- [ ] Add save button and wire to `PUT /api/settings/spam`
- [ ] Add i18n keys to all 13 locale files
...
- [ ] Create `tests/voice-captcha.spec.ts`:
...
- [ ] CAPTCHA challenge generation returns 4 digits (1–9)
- [ ] Challenge stored in DB / KV with correct TTL
- [ ] Correct DTMF → call enqueued for volunteers
- [ ] Incorrect DTMF → retry up to `captchaMaxAttempts`, then reject
- [ ] All 5 telephony adapters handle CAPTCHA response
- [ ] Admin can toggle CAPTCHA + set max attempts in UI
- [ ] E2E tests pass: `bunx playwright test tests/voice-captcha.spec.ts`
