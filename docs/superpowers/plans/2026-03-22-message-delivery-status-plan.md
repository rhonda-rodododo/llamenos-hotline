...
- [ ] Add `deliveryStatus` enum to messages table in `src/server/db/schema/conversations.ts`:
  ```typescript
...
- [ ] Add column to `messages` table:
  ...
  ```
- [ ] Run `bunx drizzle-kit generate` and verify migration
- [ ] Add Zod schema update: `MessageRecord.deliveryStatus`, `MessageRecord.providerMessageId`
...
- [ ] Open `src/server/messaging/messaging-adapter.ts`
- [ ] Change `sendMessage()` return type from `void` to:
  ```typescript
  Promise<{ providerMessageId: string; status: 'sent' | 'failed'; error?: string }>
  ```
- [ ] Update all 3 adapter implementations (SMS, WhatsApp, Signal):
...
- [ ] Each adapter: capture Twilio MessageSid (or equivalent) and initial status from API response

### 2.2 Update message send flow
- [ ] In `ConversationService` (or DO equivalent), after `adapter.sendMessage()`:
  ```typescript
...
- [ ] In `src/server/routes/telephony.ts` or new `src/server/routes/messaging.ts`:
...
- [ ] In SMS adapter `sendMessage()` call, add:
  ```typescript
  statusCallback: `${process.env.BASE_URL}/api/messaging/status-callback`
  ```
- [ ] Document: `BASE_URL` env var must be set for status callbacks to work

### 3.3 WhatsApp / Signal status webhooks
- [ ] WhatsApp: map Meta Webhook `statuses[].status` field similarly
- [ ] Signal: Signal doesn't provide read receipts for business accounts — mark as `sent` permanently
...
- [ ] Create `src/client/components/conversations/message-status-icon.tsx`:
  ```tsx
...
- [ ] In `src/client/components/conversations/` (or existing conversation component):
...
- [ ] Subscribe to `messaging:status-updated` events in the conversation view
- [ ] On event receipt: update message status optimistically in local state

---

## Phase 5: API Updates

- [ ] Update `GET /api/conversations/:id/messages` response to include `deliveryStatus`, `deliveryStatusUpdatedAt`, `deliveryError`
...
- [ ] Add to `tests/messaging.spec.ts` or new test:
...
- [ ] `messages` table has `deliveryStatus`, `providerMessageId`, `deliveryError` columns
- [ ] `MessagingAdapter.sendMessage()` returns `{ providerMessageId, status }`
- [ ] Status callback route updates message status in DB
- [ ] `MessageStatusIcon` component renders correct icon per status
- [ ] Outbound messages show status icon in conversation thread
- [ ] i18n keys added to 13 locales
