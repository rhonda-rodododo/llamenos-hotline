...
- [x] **[Epic 361: Mobile Client Compatibility — Drizzle Backend Validation](epics/epic-361-mobile-drizzle-compat.md)** — Codegen verified: zero drift on TS/Swift/Kotlin types + i18n. Protocol schemas unchanged. Mobile build failures are pre-existing (iOS: duplicate RecordContact struct, Android: missing schema browser i18n strings) — NOT migration regressions.
