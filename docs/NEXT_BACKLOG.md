...
- [x] **Epic 360: BDD Test Parity — Fix Remaining Failures** — Fixed response shapes (notes, calls), Nostr event publishing (8 endpoints), auth validation (no auto-register), permission matrix (201 status codes), CMS state init. Results: 455→477 passed, 26 remaining assertion failures (field value mismatches)
- [ ] **[Epic 361: Mobile Client Compatibility — Drizzle Backend Validation](epics/epic-361-mobile-drizzle-compat.md)** — Verify iOS/Android/Desktop compatibility with Drizzle backend via codegen pipeline. OpenAPI snapshot diff, codegen:check, mobile build + test. Codegen drives all client types.
