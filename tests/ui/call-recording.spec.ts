...
test.describe('Call Recording Playback', () => {
  test('call history page renders recording badge and player for calls with recordings', async ({
    ...
    const hasRecordingBadge = await adminPage.getByTestId('recording-badge').count()
    const hasRecordingPlayer = await adminPage.getByTestId('recording-player').count()
  ...
  test('recording player component renders correctly when present', async ({ adminPage }) => {
    ...
    const badgeCount = await adminPage.getByTestId('recording-badge').count()

    // Both should be equal (one player per badge)
    expect(playerCount).toBe(badgeCount)
  })

  test('notes page shows recording player for calls with recordings', async ({ adminPage }) => {
    await navigateAfterLogin(adminPage, '/notes')
    await expect(adminPage.getByRole('heading', { name: 'Call Notes' })).toBeVisible()
    ...
    const recordingPlayers = await adminPage.getByTestId('recording-player').count()
    expect(recordingPlayers).toBeGreaterThanOrEqual(0)
  })

  test('recording player shows play button with correct label', async ({ adminPage }) => {
    ...
    const playerCount = await adminPage.getByTestId('recording-player').count()
    if (playerCount > 0) {
      const playBtn = adminPage.getByTestId('recording-play-btn').first()
