...
/**
 * Create a report via the UI. Assumes admin is logged in and on the reports page.
 */
async function createReportViaUI(page: Page, title: string, details: string): Promise<void> {
  ...
  await expect(page.getByPlaceholder('Brief description of the report')).not.toBeVisible({ timeout: 15000 })
 ...
 * 1. Admin creates an invite with reporter role
 * 2. A fresh Nostr keypair is generated
 * 3. The invite is redeemed with the new keypair's Schnorr signature
...
async function createReporterViaApi(request: APIRequestContext): Promise<string> {
  ...
  // Create invite with reporter role
  ...
  // Generate fresh Nostr keypair for the reporter
  const secretKey = generateSecretKey()
  const pubkey = getPublicKey(secretKey)
  const nsec = nip19.nsecEncode(secretKey)

  // Redeem the invite with Schnorr proof-of-key
